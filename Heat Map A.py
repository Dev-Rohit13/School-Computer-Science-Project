...
class Map: 
    ...
    def showMap(self, world_map):
        #Create the map
        world_map.save("map_a.html")
        webbrowser.open("map_a.html")
...
'-------------------------------------------------------------------------------------------------------------------------------------------'
def sort_spending(spending_data):
    total_spending = {}
    for country, data in spending_data.items():
        total_spending[country] = data['Refugee spending'] + data['Foreign affairs spending']
    sorted_spending = sorted(total_spending.items(), key=lambda x: x[1], reverse=True)
    return sorted_spending

sorted_spending = sort_spending(spending_data)
for country, total in sorted_spending:
    print(country, ':', total)

total_spending_per_country = []

for country, data in spending_data.items():
    total_spending = data['Refugee spending'] + data['Foreign affairs spending']
    total_spending_per_country.append(total_spending)

def normal_pdf(x, mean, std_dev):
    """Calculate the probability density function of a normal distribution."""
    return (1 / (std_dev * np.sqrt(2 * np.pi))) * np.exp(-((x - mean) ** 2) / (2 * std_dev ** 2))

mean_spending = np.mean(total_spending_per_country)
std_spending = np.std(total_spending_per_country)

print(mean_spending)
print(std_spending)

x = np.linspace(mean_spending - 3*std_spending, mean_spending + 3*std_spending, 100)
y = normal_pdf(x, mean_spending, std_spending)

plt.plot(x, y, label='Normal Curve')
plt.hist(total_spending_per_country, bins=20, density=True, alpha=0.6, color='g', label='Spending Data')

plt.xlabel('Total Spending')
plt.ylabel('Probability Density')
plt.title('Spending Data Mapped to Normal Curve')
plt.legend()
plt.show()